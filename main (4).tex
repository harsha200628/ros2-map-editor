...
% ============================================================
%  ROS2 Map Editor -- IEEE RA-L submission
%  Compile with pdflatex on Overleaf (IEEEtran.cls is preloaded
%  there). Recommended Overleaf template: "IEEE RA-L" official
%  template -- you may drop this body text into that template
%  if your submission portal requires the exact RA-L class file.
% ============================================================
...
\title{\LARGE ROS2 Map Editor: A Zero-Installation Browser Tool for Semantic Occupancy-Grid Editing, Navigation-Aware Path Preview, and ICP-Based Map Fusion}
...
flashed to the physical robot and Nav2 is restarted. \textbf{Absence of multi-session fusion support.}  Combining maps from
...
This paper presents \emph{2D Occupancy Grid Map Editor}, a single-page web application
...
\begin{enumerate}
  ...
  \item A semantic-value-aware raster editing surface constrained to $\{0,205,254\}$, with multi-level undo/redo.
  \item An 8-neighbourhood class-aware majority-vote de-noising filter that preserves unknown-space boundaries (Table~\ref{tab:noiseval}).
  \item A Nav2-inspired costmap inflation preview via octile-distance BFS, coupled with an interactive $A^{*}$ path planning preview.
  \item A 2-D ICP routine with a uniform spatial hash, achieving 91\,\% alignment success within $\pm15^{\circ}$ initial error.
  \item Integrated metric measurement and waypoint authoring with JSON export.
\end{enumerate}
...
We presented \emph{2D Occupancy Grid Map Editor}, an open-source, installation-free browser
...
The \emph{2D Occupancy Grid Map Editor} is released under the MIT License.
